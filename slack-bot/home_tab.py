...
def build_home_tab_view(
    ...
    if trial_info and not trial_info.get("expired"):
        ...
        blocks.append({
            ...
            "accessory": {
                "type": "button",
                "text": {"type": "plain_text", "text": "Add API Key", "emoji": True},
    ...
    if configured_integrations:
        for int_id, config in configured_integrations.items():
            ...
            schema = next(
                (s for s in available_schemas if s.get("id") == int_id), None
            )
    ...
    if to_show:
        ...
        blocks.append({
            "type": "section",
            "text": {"type": "mrkdwn", "text": "_All available integrations are connected!_"},
        })
