...
if ! kubectl cluster-info &> /dev/null; then
    ...
    echo "Please run: aws eks update-kubeconfig --name incidentfox-demo --region us-west-2"
    exit 1
fi
...
if ! kubectl get ns incidentfox &> /dev/null; then
    echo -e "${RED}❌ incidentfox namespace not found${NC}"
    exit 1
fi
echo "✅ incidentfox namespace exists"
