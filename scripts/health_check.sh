#!/bin/bash
#
# IncidentFox Quick Health Check
# 
...
VERBOSE=${1:-""}
NAMESPACE=${NAMESPACE:-incidentfox}
WEB_UI_URL=${WEB_UI_URL:-https://ui.incidentfox.ai}
...
check_pods() {
    ...
    STATUS=$(kubectl get pods -n $NAMESPACE -l app=incidentfox-$name -o jsonpath='{.items[0].status.phase}' 2>/dev/null)
    ...
    if [[ "$STATUS" == "Running" ]]; then
        ...
        ((FAILED++))
        if [[ "$VERBOSE" == "--verbose" ]]; then
            kubectl get pods -n $NAMESPACE -l app=incidentfox-$name 2>&1 | sed 's/^/  /'
        fi
    fi
}
...
echo -e "\n${YELLOW}── Services ──${NC}"
check "agent service" "kubectl get svc incidentfox-agent -n $NAMESPACE"
check "config-service service" "kubectl get svc incidentfox-config-service -n $NAMESPACE"
check "orchestrator service" "kubectl get svc incidentfox-orchestrator -n $NAMESPACE"
check "web-ui service" "kubectl get svc incidentfox-web-ui -n $NAMESPACE"

echo -e "\n${YELLOW}── Ingress ──${NC}"
check "ingress exists" "kubectl get ingress incidentfox-web-ui -n $NAMESPACE"
ALB_ADDR=$(kubectl get ingress incidentfox-web-ui -n $NAMESPACE -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null)
if [[ -n "$ALB_ADDR" ]]; then
...
check "DNS resolves" "nslookup ui.incidentfox.ai"
