...
project = "incidentfox-config-service"
...
if [ -z "$JUMPBOX_ID" ]; then
  # Find jumpbox by tag name (created by the RDS stack): Name=incidentfox-config-service-jumpbox
  JUMPBOX_ID="$(
    aws --profile "$AWS_PROFILE" --region "$AWS_REGION" ec2 describe-instances \
...
if [ -z "$JUMPBOX_ID" ] || [ "$JUMPBOX_ID" = "None" ]; then
  ...
  echo "Expected tag: Name=incidentfox-config-service-jumpbox"
