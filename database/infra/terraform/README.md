## Terraform: IncidentFox Config Service (RDS PostgreSQL)

This Terraform module provisions an **AWS RDS PostgreSQL** instance inside an **existing VPC**, intended to be used by `incidentfox-config-service` running inside the same VPC (e.g., alongside your OTel demo microservices).
...
2) Edit `terraform.tfvars` (VPC + private subnets already filled for `incidentfox-demo-vpc`).
