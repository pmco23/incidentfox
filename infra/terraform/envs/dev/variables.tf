...
# Optional: create ECR repos (dev/pilot helper). Enterprises can use their own registry.
variable "create_ecr" {
  type    = bool
  default = false
}
...
# VPC mode (optional helper for dev/pilot stacks)
variable "create_vpc" {
  type    = bool
  default = false
}
...
locals {
  # Validation for BYO network mode.
  _byo_network_ok = var.create_vpc || (length(trimspace(var.vpc_id)) > 0 && length(var.private_subnet_ids) > 0)
}

# Terraform doesn't support "required unless" directly on variables; enforce via a no-op resource precondition.
