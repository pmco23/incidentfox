...
SECRET_NAME="${SECRET_NAME:-incidentfox-config-service/token_pepper}"
...
aws --profile "$AWS_PROFILE" --region "$AWS_REGION" secretsmanager create-secret \
  --name "$SECRET_NAME" \
  --description "incidentfox-config-service TOKEN_PEPPER (server-side pepper for hashing opaque bearer tokens)" \
