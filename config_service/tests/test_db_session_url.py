...
def test_get_database_url_assembles_from_components(monkeypatch):
    ...
    monkeypatch.setenv("DB_NAME", "incidentfox_config")
    monkeypatch.setenv("DB_USERNAME", "incidentfox")
    monkeypatch.setenv("DB_PASSWORD", "p@ss word/with:chars")
    monkeypatch.setenv("DB_SSLMODE", "require")

    url = get_database_url()
    assert url.startswith("postgresql+psycopg2://incidentfox:")
    assert "@rds.internal:5432/incidentfox_config?sslmode=require" in url
