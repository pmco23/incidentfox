...
def test_admin_oidc_permissions_derived_from_groups(monkeypatch):
    ...
    monkeypatch.setenv("OIDC_ADMIN_GROUP", "incidentfox-config-admin")

    monkeypatch.setenv("ADMIN_PERMISSIONS_DEFAULT", "admin:read")
    monkeypatch.setenv(
        "ADMIN_GROUP_PERMISSIONS_JSON",
        '{"incidentfox-config-admin":["admin:provision","admin:agent:run"],"incidentfox-auditor":"admin:audit"}',
    )
    ...
    def fake_verify(_token: str, *, cfg):  # noqa: ANN001
        return {
            ...
            "groups": ["incidentfox-config-admin", "incidentfox-auditor"],
        }
