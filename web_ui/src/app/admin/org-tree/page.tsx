...
const nodeColors = {
  ...
  team: { bg: 'bg-blue-600', border: 'border-blue-500', text: 'text-blue-600' },
};
...
function TreeNodeComponent({ 
  ...
  return (
    <div className="relative">
      ...
      {level > 0 && (
        <div className="absolute left-[-20px] top-0 w-[20px] h-[24px] border-l-2 border-b-2 border-gray-300 dark:border-gray-700 rounded-bl-lg" />
      )}
      ...
      <div className="flex items-center gap-2 mb-2">
        {hasChildren && (
          <button
            ...
            className="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-transform"
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 ${colors.border} bg-white dark:bg-gray-900 hover:shadow-md transition-all cursor-pointer`}
        >
      ...
      {hasChildren && expanded && (
        <div className="ml-8 pl-4 border-l-2 border-gray-200 dark:border-gray-800">
...
function Modal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-2xl max-w-lg w-full mx-4 max-h-[90vh] overflow-auto">
      ...
      <div className="p-8 max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Network className="w-7 h-7 text-gray-500" />
            <div>
              <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Organization Tree</h1>
              <p className="text-sm text-gray-500">Manage teams and hierarchy for <span className="font-mono">{orgId}</span></p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => { setNewNodeParent('root'); setAddNodeOpen(true); }}
              className="flex items-center gap-2 px-3 py-2 text-sm font-medium bg-orange-600 text-white rounded-lg hover:bg-orange-700"
            >
        ...
        <div className="flex items-center gap-6 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-600 dark:text-gray-400">Unit (Group)</span>
          </div>
          <div className="flex items-center gap-2">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-800">
                    ...
                    <span className="font-mono text-gray-900 dark:text-white">{selectedNode.node_id}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-800">
                    <span className="text-gray-500">Parent</span>
                    <span className="font-mono text-gray-900 dark:text-white">{selectedNode.parent_id || '(root)'}</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-1 px-3 py-2 text-sm bg-orange-600 text-white rounded-lg hover:bg-orange-700"
                  >
                    <Plus className="w-4 h-4" /> Add Child
                  </button>
                  <button
                    onClick={openEditNode}
                    className="flex items-center justify-center gap-1 px-3 py-2 text-sm bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700"
                  >
        ...
        <Modal isOpen={addNodeOpen} onClose={() => setAddNodeOpen(false)} title="Add New Node">
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Node ID</label>
              <input
                value={newNodeId}
            ...
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Parent Node</label>
              <select
                value={newNodeParent}
        ...
        <Modal isOpen={editNodeOpen} onClose={() => setEditNodeOpen(false)} title="Edit Node">
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Node ID (read-only)</label>
              <input
                value={selectedNode?.node_id || ''}
                disabled
                className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 opacity-60"
              />
            </div>
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Display Name</label>
        ...
        <Modal isOpen={configOpen} onClose={() => setConfigOpen(false)} title={`Config: ${selectedNode?.node_id}`}>
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Effective Config (inherited)</label>
              <pre className="text-xs bg-gray-50 dark:bg-gray-950 p-3 rounded-lg overflow-auto max-h-40 border border-gray-200 dark:border-gray-800">
                {effectiveConfig ? JSON.stringify(effectiveConfig, null, 2) : 'Loading...'}
        </pre>
            </div>
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Local Config (JSON)</label>
              <textarea
                value={configPatch}
                onChange={(e) => setConfigPatch(e.target.value)}
                rows={6}
                className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 font-mono text-xs"
              />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button
        ...
        <Modal isOpen={tokensOpen} onClose={() => setTokensOpen(false)} title={`Tokens: ${selectedNode?.node_id}`}>
          <div className="space-y-4">
            ...
            {tokens.length === 0 ? (
              ...
              <div className="space-y-2">
                {tokens.map((t) => (
                  <div key={t.token_id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <div>
                      <div className="font-mono text-xs text-gray-900 dark:text-white">{t.token_id}</div>
