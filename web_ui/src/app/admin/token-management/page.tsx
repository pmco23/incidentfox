...
export default function TokenManagementPage() {
  ...
  const getStatusBadge = (status: TokenStatus) => {
    switch (status) {
      ...
      case 'unused':
        return <span className="inline-flex items-center gap-1 text-xs font-medium text-gray-700 bg-gray-50 dark:bg-gray-900/20 px-2 py-0.5 rounded-full"><AlertCircle className="w-3 h-3" /> Unused</span>;
      case 'revoked':
        return <span className="inline-flex items-center gap-1 text-xs font-medium text-red-700 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded-full"><XCircle className="w-3 h-3" /> Revoked</span>;
    }
  };
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Token Management</h1>
              <p className="text-sm text-gray-500">Manage authentication tokens across all teams</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 shadow-sm">
            ...
            <div className="text-3xl font-bold text-gray-900 dark:text-white">{health.active}</div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 shadow-sm">
            ...
            <div className="text-3xl font-bold text-gray-900 dark:text-white">{health.expiring}</div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 shadow-sm">
            ...
            <div className="text-3xl font-bold text-gray-900 dark:text-white">{health.unused}</div>
            <p className="text-xs text-gray-500 mt-1">Never used</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 shadow-sm">
            ...
            <div className="text-3xl font-bold text-gray-900 dark:text-white">{health.revoked}</div>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-950 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
            ...
            <select
              ...
              className="px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-950 focus:outline-none focus:ring-2 focus:ring-orange-500"
            >
            ...
            <button
              onClick={loadTokens}
              disabled={isLoading}
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-50"
            >
        ...
        {selectedTokens.size > 0 && (
          <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-900/40 rounded-xl p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-orange-900 dark:text-orange-100">
                {selectedTokens.size} token{selectedTokens.size > 1 ? 's' : ''} selected
              </span>
              <div className="flex gap-2">
                <button className="px-3 py-1.5 text-sm font-medium text-red-700 bg-red-100 dark:bg-red-900/20 rounded-lg hover:bg-red-200">
                  Revoke Selected
                </button>
                <button className="px-3 py-1.5 text-sm font-medium text-gray-700 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200">
                  Export to CSV
                </button>
              </div>
            </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filteredTokens.length === 0 ? (
                  ...
                  filteredTokens.map((token) => (
                    <tr key={token.token_id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{token.team_name || token.team_node_id}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{formatDate(token.issued_at)}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{token.expires_at ? formatDate(token.expires_at) : 'Never'}</td>
                      <td className="px-4 py-3 text-sm text-gray-500">{formatRelativeTime(token.last_used_at)}</td>
        ...
        {issueModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setIssueModalOpen(false)}>
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-800">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Issue New Token</h2>
              ...
              <div className="p-6 space-y-4">
                {issueStep === 1 && (
                  <>
                    <div className="text-sm text-gray-600 dark:text-gray-400 mb-4">Step 1 of 3: Select Team</div>
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500"
                      >
                ...
                {issueStep === 2 && (
                  <>
                    <div className="text-sm text-gray-600 dark:text-gray-400 mb-4">Step 2 of 3: Set Expiration</div>
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Token expiration:</label>
                      {[30, 60, 90, 180, 365, null].map((days) => (
                        <label key={days || 'never'} className="flex items-center gap-3 p-3 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
                          <input
                            type="radio"
                ...
                {issueStep === 3 && !issuedTokenSecret && (
                  <>
                    <div className="text-sm text-gray-600 dark:text-gray-400 mb-4">Step 3 of 3: Review</div>
                ...
                {issuedTokenSecret && (
                  <>
                    <div className="text-sm text-gray-600 dark:text-gray-400 mb-4">Step 3 of 3: Copy Token</div>
                    <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-900/40 rounded-lg p-4">
                      ...
                      <div className="flex items-center gap-2">
                        <code className="flex-1 bg-white dark:bg-gray-900 border border-yellow-200 dark:border-yellow-900/50 rounded p-3 text-xs font-mono break-all">
                          {issuedTokenSecret}
                        </code>
                        <button
                          onClick={() => navigator.clipboard.writeText(issuedTokenSecret)}
                          className="flex items-center gap-2 px-3 py-2 text-sm font-medium bg-white dark:bg-gray-900 border border-yellow-200 dark:border-yellow-900/50 rounded-lg hover:bg-yellow-50"
                        >
                    ...
                    <label className="flex items-center gap-2 mt-4">
                      <input
                        ...
                        className="rounded border-gray-300 text-orange-600 focus:ring-orange-500"
                      />
                      <span className="text-sm text-gray-700 dark:text-gray-300">I have saved this token securely</span>
                    </label>
                  </>
                )}
              </div>
              ...
              <div className="flex justify-end gap-2 p-6 border-t border-gray-200 dark:border-gray-800">
                {issueStep > 1 && !issuedTokenSecret && (
                  <button
                    ...
                    className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200"
                  >
                    ← Back
                  </button>
                )}
        ...
        {tokenDetailsModalOpen && selectedToken && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setTokenDetailsModalOpen(false)}>
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-800">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Token Details: {selectedToken.token_id}</h2>
                <button onClick={() => setTokenDetailsModalOpen(false)} className="text-gray-400 hover:text-gray-600">✕</button>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Token ID:</span><span className="font-mono">{selectedToken.token_id}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Team:</span><span>{selectedToken.team_name || selectedToken.team_node_id}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Issued By:</span><span>{selectedToken.issued_by}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Issued Date:</span><span>{formatDate(selectedToken.issued_at)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Expires Date:</span><span>{selectedToken.expires_at ? formatDate(selectedToken.expires_at) : 'Never'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Status:</span><span>{getStatusBadge(selectedToken.status)}</span></div>
                  </div>
                </div>

                <div>
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">📊 Usage Statistics</h3>
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Total Requests:</span><span className="font-semibold">{selectedToken.total_requests?.toLocaleString() || 0}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">Last Used:</span><span>{formatRelativeTime(selectedToken.last_used_at)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-600 dark:text-gray-400">First Used:</span><span>{formatDate(selectedToken.issued_at)}</span></div>
                  </div>
                </div>

                <div className="flex gap-2">
                  <button className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200">
                    View Full Audit Log
                  </button>
                  {!selectedToken.revoked_at && (
                    <>
                      <button className="px-4 py-2 text-sm font-medium text-orange-700 bg-orange-100 dark:bg-orange-900/20 rounded-lg hover:bg-orange-200">
                        Extend Expiry
                      </button>
                      <button
                        onClick={() => {
                        ...
                        className="px-4 py-2 text-sm font-medium text-red-700 bg-red-100 dark:bg-red-900/20 rounded-lg hover:bg-red-200"
                      >
                        Revoke Token
                      </button>
                    </>
