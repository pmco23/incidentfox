...
const sourceColors: Record<string, { bg: string; text: string; icon: any }> = {
  token: { bg: 'bg-gray-100 dark:bg-gray-800', text: 'text-gray-700 dark:text-gray-300', icon: Key },
  config: { bg: 'bg-gray-100 dark:bg-gray-800', text: 'text-gray-700 dark:text-gray-300', icon: Settings },
  agent: { bg: 'bg-gray-100 dark:bg-gray-800', text: 'text-gray-700 dark:text-gray-300', icon: Bot },
};
...
const eventTypeIcons: Record<string, { icon: any; color: string }> = {
  ...
  permission_denied: { icon: AlertTriangle, color: 'text-red-500' },
  config_updated: { icon: Settings, color: 'text-blue-500' },
  agent_completed: { icon: CheckCircle, color: 'text-green-500' },
  agent_failed: { icon: XCircle, color: 'text-red-500' },
  agent_timeout: { icon: Clock, color: 'text-orange-500' },
  agent_running: { icon: Activity, color: 'text-blue-500' },
};
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={exportCSV}
            className="flex items-center gap-2 px-4 py-2 text-sm bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-colors"
          >
        ...
        <div className="flex flex-wrap items-center gap-4">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-full transition-all ${
                  sources.includes(source)
                    ? `${style.bg} ${style.text} ring-2 ring-offset-2 ring-offset-white dark:ring-offset-gray-900 ${style.text.replace('text-', 'ring-')}`
          ...
          <div className="flex-1 relative min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 placeholder-gray-400"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-8 text-center">
            <RefreshCcw className="w-6 h-6 animate-spin mx-auto text-gray-400" />
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {events.map((event) => {
              ...
              const eventIcon = eventTypeIcons[event.event_type] || { icon: Activity, color: 'text-gray-400' };
              const isExpanded = expandedEvent === event.id;

              return (
                <div key={event.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  <button
                      ...
                      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                        ...
                        {event.correlation_id && (
                          <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <code className="text-xs font-mono text-gray-700 dark:text-gray-300">{event.correlation_id}</code>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        {total > limit && (
          <div className="flex items-center justify-between p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
            <button
              ...
              disabled={offset === 0}
              className="px-4 py-2 text-sm rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-700"
            >
              Previous
            </button>
